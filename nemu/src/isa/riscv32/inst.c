...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, uint32_t shift=*csr_no(imm);if(R(dest)!=0x0) {R(dest)=shift;};*csr_no(imm)=src1;);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, uint32_t shift=*csr_no(imm);if(R(dest)!=0x0) {R(dest)=shift;};*csr_no(imm)=(src1 | shift););
  ...
  //TYPE_N
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, printf("reach ebreak!\n");NEMUTRAP(s->pc, R(10))); // R(10) is $a0
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall , N, printf("reach ecall!\n");(s->dnpc)=isa_raise_intr(11,s->pc);printf("dnpc=%#x\n",s->dnpc););
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret , N, printf("reach mret!\n");(s->dnpc)=(cpu.mepc));
