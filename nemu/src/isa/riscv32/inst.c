...
#define immB() do { *imm = SEXT((BITS(i,31,31)<<12) | (BITS(i,30,25)<<5) | (BITS(i,11,7)<<1) | (BITS(i,7,7)<<11),12); } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, int s1=src1; R(dest) = s1 >> src2);
  ...
  //TYPE_B
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if(src1==src2) {(s->dnpc)=(s->pc)+(imm<<1); R(dest)=s->pc+4;});
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, int im=imm<<1;if(src1!=src2) {(s->dnpc)=(s->pc)+im; R(dest)=s->pc+4;});
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, int s1=src1; int s2=src2; if(s1<s2) {(s->dnpc)=(s->pc)+(imm<<1); R(dest)=s->pc+4;});
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, int s1=src1; int s2=src2; if(s1>=s2) {(s->dnpc)=(s->pc)+(imm<<1); R(dest)=s->pc+4;});
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, if(src1<src2) {(s->dnpc)=(s->pc)+(imm<<1); R(dest)=s->pc+4;});
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, if(src1>=src2) {(s->dnpc)=(s->pc)+(imm<<1); R(dest)=s->pc+4;});
  //TYPE_U
