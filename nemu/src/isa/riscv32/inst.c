...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, uint32_t shift=*csr_no(imm);R(dest)=shift;*csr_no(imm)=src1;);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, uint32_t shift=*csr_no(imm);R(dest)=shift;*csr_no(imm)=(src1 | shift););
