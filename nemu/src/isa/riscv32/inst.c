...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(dest) = src1<<(imm & 0x0000001f));//waiting for test--yhy
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = src1>>(imm & 0x0000001f));
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, int im=imm & 0x0000001f;R(dest) = src1>>im);
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, int s1=src1; int s2=src2; if(s1<s2) {(s->dnpc)=(s->pc)+(imm<<1);});
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, int s1=src1; int s2=src2; if(s1>=s2) {(s->dnpc)=(s->pc)+(imm<<1);});
