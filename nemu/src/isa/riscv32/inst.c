...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, int im=src1*src2; R(dest)= im);
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, int s1 =src1;int s2=src2; long long im0=s1*s2;int im1=im0>>32; R(dest)= im1);
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, int im=imm & 0x0000001f;int s1=src1; R(dest) = s1>>im);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, (s->dnpc)=src1+(imm); R(dest)=s->snpc+4);
