...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(dest) = (SEXT(imm,20)<<12)+(s->pc));
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", slli   , I, R(dest) = R(src1)<<(imm & 0x0000001f));//waiting for test--yhy
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, (s->dnpc)=R(src1)+(SEXT(imm,20)<<1); R(dest)=s->pc+4);

  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, (s->dnpc)=(s->pc)+(SEXT(imm,20)<<1); R(dest)=s->pc+4);
