...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00011 11", csrrw  , I, printf("reach csrrw!\n");uint32_t shift=*csr_no(imm);if(R(dest)!=0x0) {R(dest)=shift;};*csr_no(imm)=src1;);
  INSTPAT("??????? ????? ????? 010 ????? 00011 11", csrrs  , I, printf("reach csrrs!\n");uint32_t shift=*csr_no(imm);if(R(dest)!=0x0) {R(dest)=shift;};*csr_no(imm)=(src1 | shift););
